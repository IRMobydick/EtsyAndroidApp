...
public class ShopCard$$Parcelable implements android.os.Parcelable, ax<ShopCard> {
    public static final av CREATOR;
    private ShopCard shopCard$$16;

    static {
        CREATOR = new av();
    }

    public ShopCard$$Parcelable(Parcel parcel) {
        ShopCard shopCard;
        if (parcel.readInt() == -1) {
            shopCard = null;
        } else {
    ...
    public void writeToParcel(Parcel parcel, int i) {
        if (this.shopCard$$16 == null) {
            parcel.writeInt(-1);
            return;
        }
        parcel.writeInt(1);
        writecom_etsy_android_lib_models_apiv3_ShopCard(this.shopCard$$16, parcel, i);
    }
    ...
    private LocalStore readcom_etsy_android_lib_models_LocalStore(Parcel parcel) {
        ...
        if (readInt < 0) {
            ...
            arrayList = new ArrayList();
            for (i = 0; i < readInt; i++) {
                arrayList.add((LocalStoreImage) parcel.readParcelable(ShopCard$$Parcelable.class.getClassLoader()));
            }
    ...
    private LocalMarketCard readcom_etsy_android_lib_models_LocalMarketCard(Parcel parcel) {
        ...
        if (i >= 0) {
            ...
            for (int i2 = 0; i2 < i; i2++) {
                list2.add((LocalStoreImage) parcel.readParcelable(ShopCard$$Parcelable.class.getClassLoader()));
            }
        }
    ...
    private ListingCard readcom_etsy_android_lib_models_apiv3_ListingCard(Parcel parcel) {
        ...
        listingCard.mFundOnEtsyCampaign = (FundOnEtsyCampaign) parcel.readParcelable(ShopCard$$Parcelable.class.getClassLoader());
        if (parcel.readInt() != -1) {
            etsyId2 = readcom_etsy_android_lib_models_datatypes_EtsyId(parcel);
        }
