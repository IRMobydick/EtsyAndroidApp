...
final class MultiDexExtractor {
    ...
    final class C00551 implements FileFilter {
        ...
        try {
            ...
            while (entry != null) {
                ...
                while (i2 < MAX_EXTRACT_ATTEMPTS && !z) {
                    ...
                    Log.i(TAG, "Extraction " + (verifyZipFile ? BaseMessage.TYPE_SUCCESS : "failed") + " - length " + file3.getAbsolutePath() + ": " + file3.length());
                    if (!verifyZipFile) {
                        file3.delete();
                        if (file3.exists()) {
    ...
    private static void prepareDexDir(File file, String str) {
        ...
        mkdirChecked(file);
        File[] listFiles = file.listFiles(new C00551(str));
        if (listFiles == null) {
