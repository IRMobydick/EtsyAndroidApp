...
class PathInterpolatorDonut implements Interpolator {
    ...
    private static Path createQuad(float f, float f2) {
        ...
        path.quadTo(f, f2, FullImageView.ASPECT_RATIO_SQUARE, FullImageView.ASPECT_RATIO_SQUARE);
        return path;
    }
    ...
    private static Path createCubic(float f, float f2, float f3, float f4) {
        Path path = new Path();
        path.moveTo(0.0f, 0.0f);
        path.cubicTo(f, f2, f3, f4, FullImageView.ASPECT_RATIO_SQUARE, FullImageView.ASPECT_RATIO_SQUARE);
        return path;
    }
}
