...
public class ViewPager extends ViewGroup {
    ...
    final class C01631 implements Comparator<ItemInfo> {
        C01631() {
        }
    ...
    final class C01642 implements Interpolator {
        C01642() {
        }
    ...
    class C01664 implements OnApplyWindowInsetsListener {
        private final Rect mTempRect;
    ...
    public class SavedState extends BaseSavedState {
        ...
        final class C01671 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C01671() {
            }
        ...
        static {
            CREATOR = ParcelableCompat.newCreator(new C01671());
        }
    ...
    static {
        ...
        LAYOUT_ATTRS = iArr;
        COMPARATOR = new C01631();
        sInterpolator = new C01642();
    ...
    public ViewPager(Context context) {
        ...
        this.mEndScrollRunnable = new C01653();
    ...
    void initViewPager() {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(this, new C01664());
    }
    ...
    void smoothScrollTo(int i, int i2, int i3) {
        ...
        float distanceInfluenceForSnapDuration = (((float) i7) * distanceInfluenceForSnapDuration(Math.min(FullImageView.ASPECT_RATIO_SQUARE, (((float) Math.abs(i5)) * FullImageView.ASPECT_RATIO_SQUARE) / ((float) currX)))) + ((float) i7);
        int abs = Math.abs(i3);
        if (abs > 0) {
            currX = Math.round(1000.0f * Math.abs(distanceInfluenceForSnapDuration / ((float) abs))) * 4;
        } else {
            currX = (int) (((((float) Math.abs(i5)) / ((((float) currX) * this.mAdapter.getPageWidth(this.mCurItem)) + ((float) this.mPageMargin))) + FullImageView.ASPECT_RATIO_SQUARE) * 100.0f);
        }
    ...
    private void calculatePageOffsets(ItemInfo itemInfo, int i, ItemInfo itemInfo2) {
        ...
        this.mLastOffset = itemInfo.position == count + INVALID_POINTER ? (itemInfo.offset + itemInfo.widthFactor) - FullImageView.ASPECT_RATIO_SQUARE : AutoScrollHelper.NO_MAX;
        ...
        for (i3 = i + SCROLL_STATE_DRAGGING; i3 < size; i3 += SCROLL_STATE_DRAGGING) {
            ...
            if (itemInfo3.position == count + INVALID_POINTER) {
                this.mLastOffset = (itemInfo3.widthFactor + f2) - FullImageView.ASPECT_RATIO_SQUARE;
            }
            itemInfo3.offset = f2;
            f3 = f2 + (itemInfo3.widthFactor + f);
            i2 += SCROLL_STATE_DRAGGING;
        ...
        switch (motionEvent.getAction() & ValidationUtils.APPBOY_STRING_MAX_LENGTH) {
            ...
            case SCROLL_STATE_DRAGGING /*1*/:
                if (this.mIsBeingDragged) {
                    ...
                    velocityTracker.computeCurrentVelocity(Constants.APPBOY_MINIMUM_NOTIFICATION_DURATION_MILLIS, (float) this.mMaximumVelocity);
    ...
    public void draw(Canvas canvas) {
        ...
        if (overScrollMode == 0 || (overScrollMode == SCROLL_STATE_DRAGGING && this.mAdapter != null && this.mAdapter.getCount() > SCROLL_STATE_DRAGGING)) {
            ...
            if (!this.mRightEdge.isFinished()) {
                ...
                canvas.rotate(90.0f);
                canvas.translate((float) (-getPaddingTop()), (-(this.mLastOffset + FullImageView.ASPECT_RATIO_SQUARE)) * ((float) height));
