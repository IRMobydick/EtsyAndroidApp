...
public final class ColorUtils {
    ...
    private static int compositeAlpha(int i, int i2) {
        return 255 - (((255 - i2) * (255 - i)) / ValidationUtils.APPBOY_STRING_MAX_LENGTH);
    }
    ...
    private static int compositeComponent(int i, int i2, int i3, int i4, int i5) {
        ...
        return (((i * ValidationUtils.APPBOY_STRING_MAX_LENGTH) * i2) + ((i3 * i4) * (255 - i2))) / (i5 * ValidationUtils.APPBOY_STRING_MAX_LENGTH);
    }
    ...
    public static double calculateContrast(@ColorInt int i, @ColorInt int i2) {
        if (Color.alpha(i2) != ValidationUtils.APPBOY_STRING_MAX_LENGTH) {
            throw new IllegalArgumentException("background can not be translucent: #" + Integer.toHexString(i2));
        }
        if (Color.alpha(i) < ValidationUtils.APPBOY_STRING_MAX_LENGTH) {
    ...
    public static int calculateMinimumAlpha(@ColorInt int i, @ColorInt int i2, float f) {
        ...
        if (Color.alpha(i2) != ValidationUtils.APPBOY_STRING_MAX_LENGTH) {
            throw new IllegalArgumentException("background can not be translucent: #" + Integer.toHexString(i2));
        } else if (calculateContrast(setAlphaComponent(i, ValidationUtils.APPBOY_STRING_MAX_LENGTH), i2) < ((double) f)) {
            return -1;
        } else {
            int i5 = 0;
    ...
    public static void RGBToHSL(@IntRange(from = 0, to = 255) int i, @IntRange(from = 0, to = 255) int i2, @IntRange(from = 0, to = 255) int i3, @NonNull float[] fArr) {
        ...
        if (max == min) {
            ...
            f2 = f4 / (FullImageView.ASPECT_RATIO_SQUARE - Math.abs((2.0f * f5) - FullImageView.ASPECT_RATIO_SQUARE));
        }
        f = (f * 60.0f) % 360.0f;
        if (f < 0.0f) {
            f += 360.0f;
        ...
        fArr[MIN_ALPHA_SEARCH_PRECISION] = constrain(f2, 0.0f, (float) FullImageView.ASPECT_RATIO_SQUARE);
        fArr[2] = constrain(f5, 0.0f, (float) FullImageView.ASPECT_RATIO_SQUARE);
    }
    ...
    public static int HSLToColor(@NonNull float[] fArr) {
        ...
        float f3 = fArr[2];
        float abs = (FullImageView.ASPECT_RATIO_SQUARE - Math.abs((2.0f * f3) - FullImageView.ASPECT_RATIO_SQUARE)) * f2;
        float f4 = f3 - (0.5f * abs);
        float abs2 = abs * (FullImageView.ASPECT_RATIO_SQUARE - Math.abs(((f / 60.0f) % 2.0f) - FullImageView.ASPECT_RATIO_SQUARE));
        switch (((int) f) / 60) {
        ...
        return Color.rgb(constrain(round, 0, (int) ValidationUtils.APPBOY_STRING_MAX_LENGTH), constrain(round2, 0, (int) ValidationUtils.APPBOY_STRING_MAX_LENGTH), constrain(round3, 0, (int) ValidationUtils.APPBOY_STRING_MAX_LENGTH));
    }

    @ColorInt
    public static int setAlphaComponent(@ColorInt int i, @IntRange(from = 0, to = 255) int i2) {
        if (i2 >= 0 && i2 <= ValidationUtils.APPBOY_STRING_MAX_LENGTH) {
    ...
    public static int XYZToColor(@FloatRange(from = 0.0d, to = 95.047d) double d, @FloatRange(from = 0.0d, to = 100.0d) double d2, @FloatRange(from = 0.0d, to = 108.883d) double d3) {
        ...
        return Color.rgb(constrain((int) Math.round((d4 > 0.0031308d ? (Math.pow(d4, 0.4166666666666667d) * 1.055d) - 0.055d : d4 * 12.92d) * 255.0d), 0, (int) ValidationUtils.APPBOY_STRING_MAX_LENGTH), constrain((int) Math.round((d5 > 0.0031308d ? (1.055d * Math.pow(d5, 0.4166666666666667d)) - 0.055d : 12.92d * d5) * 255.0d), 0, (int) ValidationUtils.APPBOY_STRING_MAX_LENGTH), constrain((int) Math.round((d6 > 0.0031308d ? (1.055d * Math.pow(d6, 0.4166666666666667d)) - 0.055d : 12.92d * d6) * 255.0d), 0, (int) ValidationUtils.APPBOY_STRING_MAX_LENGTH));
    }
