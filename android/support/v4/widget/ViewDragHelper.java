...
public class ViewDragHelper {
    ...
    final class C02051 implements Interpolator {
        C02051() {
        }
    ...
    public static ViewDragHelper create(ViewGroup viewGroup, float f, Callback callback) {
        ...
        create.mTouchSlop = (int) (((float) create.mTouchSlop) * (FullImageView.ASPECT_RATIO_SQUARE / f));
        return create;
    }

    private ViewDragHelper(Context context, ViewGroup viewGroup, Callback callback) {
        this.mActivePointerId = INVALID_POINTER;
        this.mSetIdleRunnable = new C02062();
        if (viewGroup == null) {
    ...
    private int computeAxisDuration(int i, int i2, int i3) {
        ...
        float distanceInfluenceForSnapDuration = (distanceInfluenceForSnapDuration(Math.min(FullImageView.ASPECT_RATIO_SQUARE, ((float) Math.abs(i)) / ((float) width))) * ((float) i4)) + ((float) i4);
        i4 = Math.abs(i2);
        if (i4 > 0) {
            width = Math.round(Math.abs(distanceInfluenceForSnapDuration / ((float) i4)) * 1000.0f) * EDGE_TOP;
        } else {
            width = (int) (((((float) Math.abs(i)) / ((float) i3)) + FullImageView.ASPECT_RATIO_SQUARE) * 256.0f);
        }
    ...
    private void releaseViewForPointerUp() {
        this.mVelocityTracker.computeCurrentVelocity(Constants.APPBOY_MINIMUM_NOTIFICATION_DURATION_MILLIS, this.mMaxVelocity);
