...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    class C02411 implements MainThreadCallback<T> {
        ...
        class C02401 implements Runnable {
            ...
            public void run() {
                SyncQueueItem next = C02411.this.mQueue.next();
                while (next != null) {
                    switch (next.what) {
                        case C02411.UPDATE_ITEM_COUNT /*1*/:
                            C02411.this.val$callback.updateItemCount(next.arg1, next.arg2);
                            break;
                        case C02411.ADD_TILE /*2*/:
                            C02411.this.val$callback.addTile(next.arg1, (Tile) next.data);
                            break;
                        case C02411.REMOVE_TILE /*3*/:
                            C02411.this.val$callback.removeTile(next.arg1, next.arg2);
                            break;
                        default:
                            Log.e("ThreadUtil", "Unsupported message, what=" + next.what);
                            break;
                    }
                    next = C02411.this.mQueue.next();
                }
            }
        }

        C02411(MainThreadCallback mainThreadCallback) {
            this.val$callback = mainThreadCallback;
            this.mQueue = new MessageQueue();
            this.mMainThreadHandler = new Handler(Looper.getMainLooper());
            this.mMainThreadRunnable = new C02401();
        }
    ...
    class C02432 implements BackgroundCallback<T> {
        ...
        class C02421 implements Runnable {
            ...
            public void run() {
                while (true) {
                    SyncQueueItem next = C02432.this.mQueue.next();
                    if (next != null) {
                        switch (next.what) {
                            case C02432.REFRESH /*1*/:
                                C02432.this.mQueue.removeMessages(C02432.REFRESH);
                                C02432.this.val$callback.refresh(next.arg1);
                                break;
                            case C02432.UPDATE_RANGE /*2*/:
                                C02432.this.mQueue.removeMessages(C02432.UPDATE_RANGE);
                                C02432.this.mQueue.removeMessages(C02432.LOAD_TILE);
                                C02432.this.val$callback.updateRange(next.arg1, next.arg2, next.arg3, next.arg4, next.arg5);
                                break;
                            case C02432.LOAD_TILE /*3*/:
                                C02432.this.val$callback.loadTile(next.arg1, next.arg2);
                                break;
                            case C02432.RECYCLE_TILE /*4*/:
                                C02432.this.val$callback.recycleTile((Tile) next.data);
                                break;
                            default:
                                Log.e("ThreadUtil", "Unsupported message, what=" + next.what);
                                break;
                    ...
                    C02432.this.mBackgroundRunning.set(false);
                    return;
                }
            }
        }

        C02432(BackgroundCallback backgroundCallback) {
            ...
            this.mBackgroundRunnable = new C02421();
        }
