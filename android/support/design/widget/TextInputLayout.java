...
public class TextInputLayout extends LinearLayout {
    ...
    class C00423 extends ViewPropertyAnimatorListenerAdapter {
        final /* synthetic */ CharSequence val$error;

        C00423(CharSequence charSequence) {
    ...
    class SavedState extends BaseSavedState {
        ...
        final class C00441 implements Creator<SavedState> {
            C00441() {
            }
    ...
    public TextInputLayout(Context context, AttributeSet attributeSet, int i) {
        ...
        TypedArray obtainStyledAttributes = context.obtainStyledAttributes(attributeSet, C0000R.styleable.TextInputLayout, i, C0000R.style.Widget_Design_TextInputLayout);
        this.mHintEnabled = obtainStyledAttributes.getBoolean(C0000R.styleable.TextInputLayout_hintEnabled, true);
        setHint(obtainStyledAttributes.getText(C0000R.styleable.TextInputLayout_android_hint));
        this.mHintAnimationEnabled = obtainStyledAttributes.getBoolean(C0000R.styleable.TextInputLayout_hintAnimationEnabled, true);
        if (obtainStyledAttributes.hasValue(C0000R.styleable.TextInputLayout_android_textColorHint)) {
            ColorStateList colorStateList = obtainStyledAttributes.getColorStateList(C0000R.styleable.TextInputLayout_android_textColorHint);
            this.mFocusedTextColor = colorStateList;
            this.mDefaultTextColor = colorStateList;
        }
        if (obtainStyledAttributes.getResourceId(C0000R.styleable.TextInputLayout_hintTextAppearance, INVALID_MAX_LENGTH) != INVALID_MAX_LENGTH) {
            setHintTextAppearance(obtainStyledAttributes.getResourceId(C0000R.styleable.TextInputLayout_hintTextAppearance, 0));
        }
        this.mErrorTextAppearance = obtainStyledAttributes.getResourceId(C0000R.styleable.TextInputLayout_errorTextAppearance, 0);
        boolean z = obtainStyledAttributes.getBoolean(C0000R.styleable.TextInputLayout_errorEnabled, false);
        boolean z2 = obtainStyledAttributes.getBoolean(C0000R.styleable.TextInputLayout_counterEnabled, false);
        setCounterMaxLength(obtainStyledAttributes.getInt(C0000R.styleable.TextInputLayout_counterMaxLength, INVALID_MAX_LENGTH));
        this.mCounterTextAppearance = obtainStyledAttributes.getResourceId(C0000R.styleable.TextInputLayout_counterTextAppearance, 0);
        this.mCounterOverflowTextAppearance = obtainStyledAttributes.getResourceId(C0000R.styleable.TextInputLayout_counterOverflowTextAppearance, 0);
    ...
    private void addIndicator(TextView textView, int i) {
        if (this.mIndicatorArea == null) {
            ...
            this.mIndicatorArea.addView(new Space(getContext()), new LinearLayout.LayoutParams(0, 0, FullImageView.ASPECT_RATIO_SQUARE));
            if (this.mEditText != null) {
                adjustIndicatorPadding();
            }
        }
    ...
    public void setError(@Nullable CharSequence charSequence) {
        if (!TextUtils.equals(this.mError, charSequence)) {
            ...
            if (this.mErrorShown) {
                ...
                if (isLaidOut) {
                    if (ViewCompat.getAlpha(this.mErrorView) == FullImageView.ASPECT_RATIO_SQUARE) {
                        ViewCompat.setAlpha(this.mErrorView, 0.0f);
                    }
                    ViewCompat.animate(this.mErrorView).alpha(FullImageView.ASPECT_RATIO_SQUARE).setDuration(200).setInterpolator(AnimationUtils.LINEAR_OUT_SLOW_IN_INTERPOLATOR).setListener(new C00412()).start();
                }
            } else if (this.mErrorView.getVisibility() == 0) {
                if (isLaidOut) {
                    ViewCompat.animate(this.mErrorView).alpha(0.0f).setDuration(200).setInterpolator(AnimationUtils.FAST_OUT_LINEAR_IN_INTERPOLATOR).setListener(new C00423(charSequence)).start();
                } else {
                    this.mErrorView.setVisibility(4);
                }
            }
    ...
    private void updateCounter(int i) {
        ...
        if (this.mCounterMaxLength == INVALID_MAX_LENGTH) {
            ...
            this.mCounterView.setText(getContext().getString(C0000R.string.character_counter_pattern, new Object[]{Integer.valueOf(i), Integer.valueOf(this.mCounterMaxLength)}));
        }
    ...
    private void collapseHint(boolean z) {
        ...
        if (z && this.mHintAnimationEnabled) {
            animateToExpansionFraction(FullImageView.ASPECT_RATIO_SQUARE);
        } else {
            this.mCollapsingTextHelper.setExpansionFraction(FullImageView.ASPECT_RATIO_SQUARE);
        }
    }
