...
public class NavigationView extends ScrimInsetsFrameLayout {
    ...
    public class SavedState extends BaseSavedState {
        ...
        final class C00231 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C00231() {
            }
        ...
        static {
            CREATOR = ParcelableCompat.newCreator(new C00231());
        }
    }
    ...
    public NavigationView(Context context, AttributeSet attributeSet, int i) {
        ...
        TypedArray obtainStyledAttributes = context.obtainStyledAttributes(attributeSet, C0000R.styleable.NavigationView, i, C0000R.style.Widget_Design_NavigationView);
        setBackgroundDrawable(obtainStyledAttributes.getDrawable(C0000R.styleable.NavigationView_android_background));
        if (obtainStyledAttributes.hasValue(C0000R.styleable.NavigationView_elevation)) {
            ViewCompat.setElevation(this, (float) obtainStyledAttributes.getDimensionPixelSize(C0000R.styleable.NavigationView_elevation, 0));
        }
        ViewCompat.setFitsSystemWindows(this, obtainStyledAttributes.getBoolean(C0000R.styleable.NavigationView_android_fitsSystemWindows, false));
        this.mMaxWidth = obtainStyledAttributes.getDimensionPixelSize(C0000R.styleable.NavigationView_android_maxWidth, 0);
        if (obtainStyledAttributes.hasValue(C0000R.styleable.NavigationView_itemIconTint)) {
            colorStateList = obtainStyledAttributes.getColorStateList(C0000R.styleable.NavigationView_itemIconTint);
        } else {
            colorStateList = createDefaultColorStateList(16842808);
        }
        if (obtainStyledAttributes.hasValue(C0000R.styleable.NavigationView_itemTextAppearance)) {
            resourceId = obtainStyledAttributes.getResourceId(C0000R.styleable.NavigationView_itemTextAppearance, 0);
            int i2 = PRESENTER_NAVIGATION_VIEW_ID;
        } else {
            resourceId = 0;
            boolean z = false;
        }
        ColorStateList colorStateList2 = null;
        if (obtainStyledAttributes.hasValue(C0000R.styleable.NavigationView_itemTextColor)) {
            colorStateList2 = obtainStyledAttributes.getColorStateList(C0000R.styleable.NavigationView_itemTextColor);
        }
        if (i2 == 0 && r5 == null) {
            colorStateList2 = createDefaultColorStateList(16842806);
        }
        Drawable drawable = obtainStyledAttributes.getDrawable(C0000R.styleable.NavigationView_itemBackground);
        ...
        if (obtainStyledAttributes.hasValue(C0000R.styleable.NavigationView_menu)) {
            inflateMenu(obtainStyledAttributes.getResourceId(C0000R.styleable.NavigationView_menu, 0));
        }
        if (obtainStyledAttributes.hasValue(C0000R.styleable.NavigationView_headerLayout)) {
            inflateHeaderView(obtainStyledAttributes.getResourceId(C0000R.styleable.NavigationView_headerLayout, 0));
        }
